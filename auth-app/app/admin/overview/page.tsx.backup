            ...
            <div className="relative overflow-hidden rounded-xl border border-orange-200 bg-white/80 shadow-sm">
              ...
              <table className="min-w-full divide-y divide-orange-200 text-sm">
                ...
                <tbody className="divide-y divide-orange-100">
                  {data.users.slice(0, 20).map((user: OverviewUser, index: number) => {
                    ...
                    for (let i = 0; i < index; i++) {
                      if (data.users[i].total > user.total) {
                        rank++;
                      } else if (data.users[i].total === user.total) {
                        sameRankCount++;
                      }
                    }

                    // If there are users with the same total before this user,
                    // use the rank of the first user with this total
                    if (sameRankCount > 0) {
                      for (let i = 0; i < index; i++) {
                        if (data.users[i].total === user.total) {
                          // Calculate rank for the first occurrence
                          let firstRank = 1;
                          for (let j = 0; j < i; j++) {
                            if (data.users[j].total > user.total) {
                              firstRank++;
                            }
                          }
                          rank = firstRank;

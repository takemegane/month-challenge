...
export function setIcon(size: string, base64Data: string) {
  ...
  // In development, we could write to a file for persistence
  if (process.env.NODE_ENV === 'development') {
    try {
      ...
      console.log('Icons cached to file for development persistence');
    } catch (error) {
...
export function getIcon(size: string): string | null {
  ...
  // In development, try to load from file cache
  if (process.env.NODE_ENV === 'development') {
    try {
      const fs = require('fs');
      const path = require('path');
...
export function hasIcons(): boolean {
  return Object.keys(iconCache).length > 0 || (process.env.NODE_ENV === 'development' && checkCacheFile());
}
